...
int main(int argc, char *argv[])
{
    ...
    if(!statfs(monitoringPath, &buf))
    {
        ...
        for (fs = fstypetoname ; fs->type; fs++)
        {
            ...
            {
                printf("%s stats :\ntype of filesystem = 0x%x (%s),\noptimal transfer block size = 0x%x\ntotal data blocks in file system = 0x%x\n\nfree blocks in fs = 0x%x\nfree blocks avail to non-superuser = 0x%x\ntotal file nodes in file system = 0x%x\nfree file nodes in fs = 0x%x\nmaximum length of filenames = 0x%x\n", monitoringPath, \
                                                                                                                   buf.f_type, \
                                                                                                                   fs->name, \
                                                                                                                   buf.f_bsize, \
                                                                                                                   buf.f_blocks, \
